        ...
        renderItem={({item, index}) => (
          ...
          {index === 0 || (item.status !== data.dares[index - 1]?.status && data.dares[index - 1]?.status !== undefined) ? (
            item?.status === ACTIVE_DARE_STATUS ? 
              <LinearGradient 
                  style={[styles.linearGradient, {borderLeftColor: "#c947ff", marginRight: 10}]} 
                  start={{x: 0, y: 0}} 
                  end={{x: 0.9, y: 0}} 
              ...
              :
              <LinearGradient 
                style={[styles.linearGradient, { borderLeftColor: "rgb(137, 137, 137)", marginLeft: item?.status === ACTIVE_DARE_STATUS ? 0 : 10, marginRight: 10}]}
                start={{x: 0, y: 0}} 
                end={{x: 0.9, y: 0}} 

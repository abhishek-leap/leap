...
import {
  ...
  WHAT_IS_EMAIL_ADDRESS,
  WHAT_IS_PHONE_NUMBER,
  WINDOW_HEIGHT,
  WINDOW_WIDTH,
} from '../../../constants';
    ...
    <KeyboardAvoidingView
      ...
      <InnerView currentWidth={WINDOW_HEIGHT}>
        ...
        <SearchSection
          ...
          searchValue={value}>
          {optionChoose == 'email' ? (
            <EmailIcon color="gray" height={13} />
          ) : (
            <PhoneIcon color="gray" height={13} />
          )}
        ...
        {error != '' ? (
          <View style={{width: WINDOW_WIDTH / 1.7}}>
            <Text style={{color: color.PLAYLEAP_PINK}}>{error}</Text>
          </View>
        ) : null}
        {value == '' && isSubmit ? (
          <Text style={{color: colors.secondary}}>{'Required'}</Text>
        ) : null}
        <NextBtn
...
const SearchSection = styled.View`
  width: ${props => props.currentWidth / 1.6 + 'px'};
  height: 35px;
  border-width: 1px;
